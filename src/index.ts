...
const AVAILABLE_TEMPLATES: Template[] = [
  ...
  },
  {
    ...
    children: [
      {
        label: "React + Vite + Mantine UI",
        value: "template-react-vite-mantine",
      },
      {
        label: "React + Rspack + Mantine UI",
        value: "template-react-rspack-mantine",
      },
    ],
  },
];
...
async function promptOverwriteDirectory(
...
async function main(): Promise<void> {
  ...
  const frameworkTemplate = AVAILABLE_TEMPLATES.find(
    (template) => template.value === selectedFramework
  );
  ...
  const packageName = normalizeProjectName(
    path.basename(path.resolve(projectDirectory))
  );
  updatePackageJson(projectPath, packageName);

  console.log(
    `\n${chalk.green("✓")} ${chalk.bold.green(
